...
const ImageSplitter = ({ ...props }: Props) => {
    ...
    const [drag, setDrag] = useState<Darg>({ isDragging: false, position: splitter.startPosition });
   ...
   const refPosition=useRef(splitter.startPosition)
    ...
    const onDragMove = useCallback((event:MouseEvent) => {
        ...
        const p = (event.pageX - rectBox.left) / (rectBox.right - rectBox.left) * 100    
...
useEffect(() => {
    ...
    if (drag.isDragging) {
        ...
        refSplitter.current.addEventListener('mousemove', dragMoveHandler.current, {
            passive: false,
        });
        refSplitter.current.addEventListener('mouseup', onMouseUp);
    } else {
        refSplitter.current.removeEventListener('mousemove', dragMoveHandler.current);
    ...
    return (
        <div className='content-splitter' ref={refSplitter} >
            <div className='content-splitter-main'>
                {splitter.source}
            </div>
            <div className='content-splitter-layer' style={{ width: `${drag.position.toFixed(2)}%`  }}>
                {splitter.content}
            </div>
            <div className='splitter-divider' onMouseDown={onMouseDown} style={{ left: `${drag.position.toFixed(2)}%`}}>
                <span></span>
            </div>
        </div>
    );
